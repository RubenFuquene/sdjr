<?php

declare(strict_types=1);

namespace App\Http\Resources\Api\V1;

use Illuminate\Http\Resources\Json\JsonResource;

/**
 * @OA\Schema(
 *     schema="CommerceResource",
 *     title="Commerce Resource",
 *     description="Commerce resource representation",
 *
 *     @OA\Property(property="id", type="integer", example=1),
 *     @OA\Property(property="owner_user_id", type="integer", @OA\Items(ref="#/components/schemas/UserResource")),
 *     @OA\Property(property="department_id", type="integer", @OA\Items(ref="#/components/schemas/DepartmentResource")),
 *     @OA\Property(property="city_id", type="integer", @OA\Items(ref="#/components/schemas/CityResource")),
 *     @OA\Property(property="neighborhood_id", type="integer", @OA\Items(ref="#/components/schemas/NeighborhoodResource")),
 *     @OA\Property(property="establishment_type_id", type="integer", @OA\Items(ref="#/components/schemas/EstablishmentTypeResource")),
 *     @OA\Property(property="operates_under_franchise", type="boolean", example=false),
 *     @OA\Property(property="legal_representatives", type="array", @OA\Items(ref="#/components/schemas/LegalRepresentativeResource")),
 *     @OA\Property(property="name", type="string", example="Comercial S.A.S"),
 *     @OA\Property(property="description", type="string", example="Comercio de tecnología"),
 *     @OA\Property(property="tax_id", type="string", example="900123456"),
 *     @OA\Property(property="tax_id_type", type="string", example="NIT"),
 *     @OA\Property(property="person_type", type="string", enum={"natural","juridica"}, example="juridica", description="Derivado de tax_id_type: NIT → jurídica; CC/CE/PS → natural"),
 *     @OA\Property(property="electronic_invoicing_required", type="boolean", example=false),
 *     @OA\Property(property="address", type="string", example="Calle 123 #45-67"),
 *     @OA\Property(property="phone", type="string", example="3001234567"),
 *     @OA\Property(property="email", type="string", example="info@comercial.com"),
 *     @OA\Property(property="is_active", type="boolean", example=true),
 *     @OA\Property(property="is_verified", type="boolean", example=false),
 *     @OA\Property(property="terms_accepted_version", type="integer", example=1),
 *     @OA\Property(property="terms_accepted_at", type="string", format="date-time", example="2025-12-15T12:34:56Z"),
 *     @OA\Property(property="created_at", type="string", format="date-time", example="2025-12-15T12:34:56Z"),
 *     @OA\Property(property="updated_at", type="string", format="date-time", example="2025-12-15T12:34:56Z")
 * )
 */
class CommerceResource extends JsonResource
{
    protected ?string $uploadStatus;

    /**
     * CommerceResource constructor.
     *
     * @param  mixed  $resource
     */
    public function __construct($resource, ?string $uploadStatus = null)
    {
        parent::__construct($resource);
        $this->uploadStatus = $uploadStatus;
    }

    /**
     * Transform the resource into an array.
     *
     * @param  \Illuminate\Http\Request  $request
     */
    public function toArray($request): array
    {
        return [
            'id' => $this->id,
            'owner_user' => new UserResource($this->ownerUser),
            'department' => new DepartmentResource($this->department),
            'city' => new CityResource($this->city),
            'neighborhood' => new NeighborhoodResource($this->neighborhood),
            'establishment_type_id' => $this->establishment_type_id,
            'establishment_type' => new EstablishmentTypeResource($this->establishmentType),
            'operates_under_franchise' => $this->operates_under_franchise,
            'legal_representatives' => new LegalRepresentativeResourceCollection($this->legalRepresentativesActive),
            'documents' => new CommerceDocumentResourceCollection($this->getFilteredCommerceDocuments()),
            'name' => $this->name,
            'description' => $this->description,
            'tax_id' => $this->tax_id,
            'tax_id_type' => $this->tax_id_type,
            'person_type' => $this->person_type,
            'electronic_invoicing_required' => $this->electronic_invoicing_required,
            'address' => $this->address,
            'phone' => $this->phone,
            'email' => $this->email,
            'is_active' => $this->is_active,
            'is_verified' => $this->is_verified,
            'terms_accepted_version' => $this->terms_accepted_version,
            'terms_accepted_at' => $this->terms_accepted_at,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
        ];
    }

    /**
     * Get filtered commerce documents by upload status.
     *
     * @return \Illuminate\Support\Collection
     */
    protected function getFilteredCommerceDocuments()
    {
        if ($this->uploadStatus) {
            return $this->commerceDocuments()->where('upload_status', $this->uploadStatus)->get();
        }

        return $this->commerceDocuments;
    }
}
